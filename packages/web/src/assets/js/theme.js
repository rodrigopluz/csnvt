...
var getCurrentScreenBreakpoint =
  function getCurrentScreenBreakpoint() {
...
/* Store */
var getItemFromStore = function getItemFromStore(
  key,
  defaultValue,
) {
  var store =
...
var bgPlayerInit = function bgPlayerInit() {
  ...
  var loadVideo = function loadVideo() {
    function setupPlayer() {
      window.YT.ready(function () {
        youtubeEmbedElements.forEach(function (
          youtubeEmbedElement,
        ) {
  ...
  var adjustBackgroundRatio = function adjustBackgroundRatio() {
    var ytElements = document.querySelectorAll(
      Selector.YT_VIDEO,
    );
...
var dropdownOnHover = function dropdownOnHover() {
  var navbarArea = document.querySelectorAll(
    ...
    navbarArea.forEach(function (navbarItem) {
      navbarItem.addEventListener('mouseover', function (e) {
        if (
          ...
          var dropdownInstance = new window.bootstrap.Dropdown(
            e.target,
          );
          ...
          dropdownInstance._menu.setAttribute(
            'data-bs-popper',
            'none',
          );
...
function inertiaInit() {
  ...
  var inertiaEls = document.querySelectorAll(
    Selector.DATA_INERTIA,
  );
  inertiaEls.forEach(function (el) {
    ...
    var initialTranslate =
      ((offsetTop - currentPosition) * 100) / winHeight;
    var controller = {
      weight: 2,
      duration: 0.7,
      ease: 'Power3.easeOut',
    ...
    el.style.transform = 'translateY('.concat(
      initialTranslate,
      'px);',
    );
  ...
  document
    ...
    .forEach(function (anchor) {
      anchor.addEventListener('click', function (e) {
        ...
        var el = e.target;
        var id =
          utils.getData(el, 'scroll-to') ||
          el.getAttribute('href');
        window.scroll({
          top:
            (_utils$getData2 = utils.getData(
              el,
              'offset-top',
      ...
      var newSwiper = new window.Swiper(
        ...
        _objectSpread(
          ...
          {},
          {
            ...
            on: {
              init: function init() {
                var timelineElements = swiper.querySelectorAll(
...
var zanimation = function zanimation(el, callback) {
  ...
  /* Get Controller */
  var controllerZanim;
  var currentBreakpointName =
    utils.getCurrentScreenBreakpoint().currentBreakpoint;
  var currentBreakpointVal =
    utils.getCurrentScreenBreakpoint().breakpointStartVal;
  ...
  var getController = function getController(element) {
    ...
    if (
      element.hasAttribute(
        'data-zanim-'.concat(currentBreakpointName),
      )
    ) {
      ...
      attributes.forEach(function (attribute) {
        if (
          attribute !== DATA_KEY.DATA_ZANIM_TRIGGER &&
          attribute.startsWith('data-zanim-')
        ) {
          var breakPointName = attribute.split('data-zanim-')[1];

          if (
            utils.breakpoints[breakPointName] <
      ...
      if (animationBreakpoints.length !== 0) {
        animationBreakpoints = animationBreakpoints.sort(
          function (a, b) {
  ...
  if (zanimTimeline) {
    ...
    var timelineElements = el.querySelectorAll(
      Selector.DATA_KEYS,
    );
...
var zanimationInit = function zanimationInit() {
  ...
  var triggerZanimation = function triggerZanimation() {
    var triggerElement = document.querySelectorAll(
      Selector.DATA_ZANIM_TRIGGER,
    );
    triggerElement.forEach(function (el) {
      if (
        utils.isElementIntoView(el) &&
        el.hasAttribute(DATA_KEY.DATA_ZANIM_TRIGGER)
      ) {
        ...
        if (
          !document.querySelector(Selector.DATA_ZANIM_REPEAT)
        ) {
